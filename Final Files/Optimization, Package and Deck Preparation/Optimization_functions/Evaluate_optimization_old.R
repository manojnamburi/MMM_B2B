...
eval_optim <- function(sim_res,constr_step_size_vec = NULL){
...
#   var_n_special_period <- input_params_list$var_n_special_period
  ...
  ##Number of periods of off based on number of periods of flight
  ...
  ##Extract all the budget and check lists along with the graphs
  ...
  cat(sprintf('2. Optimization bounds chosen by excluding zero spend periods for %s media channels in mean calculated from %s periods:\n\tUpper = %s\n\tLower = %s\n',str_zero_weeks,str_qy,scales::label_percent()(1-var_optimization_lb),scales::label_percent()(1-var_optimization_ub)))
  cat(sprintf('3. Simulation bounds chosen:\n\tUpper = %s\n\tLower = %s\n',scales::label_percent()(1-var_lb_budget_sim),scales::label_percent()(1-var_ub_budget_sim)))
  ##Constrained media, flight media and special media
  ...
  #cat(sprintf('The number of periods of flight by media: %s\n',str_n_flight_period))
  ...
  cat('2.a.Checks to see if input budget and optimized budget are same - Weekly\n')
  cat('2.b.Checks to see if input footprint budget and optimized footprint budget are same - Quarterly\n')
  ...
  bs_unmatch_res_w <- list()
  bs_unmatch_res_q <- list()
  bs_target_non_incr <- list()
  
  
  ps_bound_check_res <- list()
  ps_unmatch_res_w <- list()
  ps_unmatch_res_q <- list()
  ps_target_non_incr <- list()
  
  ###Code to check if the Input quarterly budget matches the Optimized Quarterly budget by using both on and off scenarios
  budget_on <- before_smoothening_budget_list[[1]]
  ##Default budget_off dataframe
  budget_off <- budget_on %>%
                dplyr::mutate_at(.vars = vars(media_names_vec),.funs = ~.*0)
    
  if(!is.null(flight_media_vec)){
      budget_off <- before_smoothening_budget_list[[2]]
  }
    
  ##Obtain the transformed quarterly level budgets by multiplying the number of flight periods with the respective scenario
  budget_on <- budget_on %>%
               dplyr::group_by(Input_quarterly_budget) %>%
               dplyr::select(media_names_vec) %>%
               dplyr::summarize_all(sum) %>%
               dplyr::mutate_at(.vars = vars(media_names_vec),.funs = ~var_n_flight_period*.) %>%
               dplyr::mutate(total_optimized_budget = rowSums(.[media_names_vec])) %>%
               dplyr::select(Input_quarterly_budget,total_optimized_budget)
    
  budget_off <- budget_off %>%
                dplyr::group_by(Input_quarterly_budget) %>%
                dplyr::select(media_names_vec) %>%
                dplyr::summarize_all(sum) %>%
                dplyr::mutate_at(.vars = vars(media_names_vec),.funs = ~var_n_off_period*.) %>%
                dplyr::mutate(total_optimized_budget = rowSums(.[media_names_vec])) %>%
                dplyr::select(Input_quarterly_budget,total_optimized_budget)
    
    
  ##Join both the scenarios and check to see if they add up to the quarterly level input budget
  budget_check_quart_bs <- budget_on %>%
                            dplyr::left_join(budget_off,by = 'Input_quarterly_budget',suffix = c('_on','_off')) %>%
                            dplyr::mutate(total_quart_opt_budget = total_optimized_budget_on + total_optimized_budget_off) %>%
                            dplyr::mutate(DMA_budget_mismatch_quart =                                                                                                                 ifelse(round(Input_quarterly_budget)==round(total_quart_opt_budget),0,1))
  ...
  for(scenario in sim_scenarios){
    ...
    cat(rep('-',60))
      cat('\n')
    cat('\t\t2.a. Constraint Check (Input weekly budget = Optimized weekly budget) - Before Smoothening\n')
    cat(rep('-',60))
      cat('\n')
    check_df <- budget_df %>% 
    ...
    bs_unmatch_res_w[[scenario]] <- unmatch_res
      
    cat(rep('-',60))
      cat('\n')
    cat('\t\t2.b. Constraint Check (Input Quarterly budget = Optimized Quarterly budget) - Before Smoothening\n')
    cat(rep('-',60))
      cat('\n')
      
    check_df <- budget_check_quart_bs %>%
                dplyr::filter(DMA_budget_mismatch_quart == 1)
      
    unmatch_res <- data.frame()
    if(nrow(check_df)!=0){
      cat('Optimized and input budget mismatch found for the input budget at the quarterly footprint level:\n')
      unmatch_res <- check_df %>%
                    dplyr::select(Input_quarterly_budget,total_quart_opt_budget) 
      unmatch_res %>%
        knitr::kable() %>%
        print()
    }else{
    ...
    bs_unmatch_res_q[[scenario]] <- unmatch_res
   ...
   cat('\t\t3. Target Monotonicity Check - Before Smoothening\n')
    ...
    if(nrow(target_not_incr_df)!=0){
      ...
      target_not_incr_df %>%
        dplyr::select(iteration_num,dma,Input_quarterly_budget,Input_DMA_Budget,dma_optimized_budget,target_var) %>%
        knitr::kable() %>%
        print()
    ...
    if(!is.null(constraint_media_vec) & is.null(var_specific_input_budget)){
      ...
      if(nrow(target_not_incr_df)!=0){
        cat('Some points found where target is not monotonically increasing:\n')
        target_not_incr_df %>%
          dplyr::select(Iteration_Num,dma,Input_quarterly_budget,Input_DMA_Budget,dma_optimized_budget_ps,target_var) %>%
          knitr::kable() %>%
          print()
}else{
        cat('Target monotonically increasing for all budget values!\n')
    ...
    #before_smoothening_sim_graphs[[scenario]]
    cat(rep('-',60))
    cat('\n')
  }##Done checks for before smoothening
  ...
  if(var_do_smoothen){
     ...
     ###Code to check if the Input quarterly budget matches the Optimized Quarterly budget by using both on and off scenarios
      budget_on <- post_smoothening_budget_list[[1]]
      ##Default budget_off dataframe
      budget_off <- budget_on %>%
                    dplyr::mutate_at(.vars = vars(media_names_vec),.funs = ~.*0)

      if(!is.null(flight_media_vec)){
          budget_off <- post_smoothening_budget_list[[2]]
      }
      ...
      ##Join both the scenarios and check to see if they add up to the quarterly level input budget
      budget_check_quart_ps <- budget_on %>%
                                dplyr::left_join(budget_off,by = 'Input_quarterly_budget',suffix = c('_on','_off')) %>%
                                dplyr::mutate(total_quart_opt_budget = total_optimized_budget_on + total_optimized_budget_off) %>%
                                dplyr::mutate(DMA_budget_mismatch_quart =                                                                                                                 ifelse(round(Input_quarterly_budget)==round(total_quart_opt_budget),0,1))
    ...
    for(scenario in sim_scenarios){
      ...
      cat(rep('-',60))
      cat('\n')
      cat('\t\t2.a. Constraint Check (Input budget = Optimized budget) - After Smoothening\n')
      cat(rep('-',60))
      cat('\n')
      ...
      ps_unmatch_res_w[[scenario]] <- unmatch_res
        
        
      cat(rep('-',60))
      cat('\n')
    cat('\t\t2.b. Constraint Check (Input Quarterly budget = Optimized Quarterly budget) - After Smoothening\n')
    cat(rep('-',60))
      cat('\n')
      
    check_df <- budget_check_quart_ps %>%
                dplyr::filter(DMA_budget_mismatch_quart == 1)
    ...
    ps_unmatch_res_q[[scenario]] <- unmatch_res
      ...
      #post_smoothening_sim_graphs[[scenario]]
      cat(rep('-',60))
      cat('\n')
    }
  cat(rep('-',60))
  ...
  output_list <- list('before_smoothening' = list('type1_issue' = bs_bound_check_res,
                                                  'type2a_issue' = bs_unmatch_res_w,
                                                  'type2b_issue' = bs_unmatch_res_q,
                                                  'type3_issue' = bs_target_non_incr),
                      'after_smoothening' = list('type1_issue' = ps_bound_check_res,
                                                 'type2a_issue' = ps_unmatch_res_w,
                                                  'type2b_issue' = ps_unmatch_res_q,
