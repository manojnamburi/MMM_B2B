...
#   if(((length(sim_results_opt[['budget']])-1)!=length(flight_media_vec_OPT))||(((length(sim_results_sdl[['budget']])-1)!=length(flight_media_vec_SDL)))){stop(cat('Please ensure you have entered the possible week combination values for the flight scenarios.'))}
  ...
  if(!is.null(constr_media_vec_OPT)||!is.null(constr_media_vec_SDL)){
    ...
    ##Rounding up the partial target values by using ceiling
    constr_target_by_dma <- sapply(constr_target_by_dma,FUN = ceiling,USE.NAMES = F)
  ...
  total_target_period <- format(ceiling((sum(media_target_vec) + sum(control_target_mat))),big.mark = ',')
  total_target_opt <- format(ceiling((sum(media_target_vec[var_id_opt]) + sum(control_target_mat[var_id_opt,]))),big.mark = ',')
  total_target_sdl <- format(ceiling((sum(media_target_vec[-var_id_opt]) + sum(control_target_mat[-var_id_opt,]))),big.mark = ',')
  ...
  fprint_target_df <- period_target_df1 %>%
    ...
    dplyr::rename('Footprint' = 'dma') %>%
    dplyr::arrange(factor(Footprint, levels = c('Altice USA','Optimum','Suddenlink'))) %>%
    dplyr::mutate_at(.vars = vars(req_cols),.funs = function(x){format(ceiling(x),big.mark = ',')}) %>%
    dplyr::rename_at(.vars = vars(media_names_vec), ~as.character(cleaned_names_list)) %>%
    dplyr::select(c('Footprint',req_cols_fmt)) %>%
    dplyr::ungroup()
  ...
  fprint_tga_df <- tga_df %>%
    ...
    dplyr::rename('Footprint' = 'dma') %>%
    dplyr::arrange(factor(Footprint, levels = c('Altice USA','Optimum','Suddenlink'))) %>%
    dplyr::mutate_at(.vars = vars('TGA'),.funs = function(x){format(ceiling(x),big.mark = ',')}) %>%
    dplyr::ungroup()
