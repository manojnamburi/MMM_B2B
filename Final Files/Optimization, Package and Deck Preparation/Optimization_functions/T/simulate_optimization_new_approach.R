...
source('Optimization_functions/T/Optimization_function.R')
source('Optimization_functions/T/DMA_Optimizer.R')
...
#'
#'The simulation limits are determined from the optimization limits by taking a smaller region to avoid edge-case non-convergency issues.
#'This means, the lower bound for simulation is set at 0.1 greater than the optimization limit and upper bound is set at 0.1 lesser value.
#'
#'Formula used to calculate the start of the simulation: round(sum(mean_spend_data[var_id_opt,-id_constraint]*var_lb_budget_sim, na.rm = TRUE)*nbr_period_q, -5)
#'Formula used to calculate the end of the simulation: round(sum(mean_spend_data[var_id_opt,-id_excl]*var_ub_budget_sim, na.rm = TRUE)*nbr_period_q, -5)
#'
#'Here, mean_spend_data is calculated based on the inputs exclude zero weeks and the input spend period.
#'The id.excl is a vector of both constrained and flight media.
#'The id.constraint is a vector of only the constrained media.
...
simulator <- function(data_XyZ,                                                ##Input data which contains spend columns
                      ...
                      flight_media_share = NULL                                    ##Share of spend of flight media
                      ){
...
#   }
  ...
  ##Get the mean spend data by the selected quarter
   mean_quarterly_data <- data_XyZ %>% 
    ...
    mean_quarterly_data_Q1 <- mean_quarterly_specific_data %>% filter(quarter == 'Q1') %>% select(media_names_vec) %>% as.matrix()
    mean_quarterly_data_Q2 <- mean_quarterly_specific_data %>% filter(quarter == 'Q2') %>% select(media_names_vec) %>% as.matrix()
    mean_quarterly_data_Q3 <- mean_quarterly_specific_data %>% filter(quarter == 'Q3') %>% select(media_names_vec) %>% as.matrix()
    mean_quarterly_data_Q4 <- mean_quarterly_specific_data %>% filter(quarter == 'Q4') %>% select(media_names_vec) %>% as.matrix()

    ##Obtain the weekly mean data by dma from the quarterly mean data
    mean_spend_data_Q1 <- sweep(mean_quarterly_data_Q1,MARGIN = 2,period_vec,'/')
    mean_spend_data_Q1 <- data.frame(mean_spend_data_Q1)

    mean_spend_data_Q2 <- sweep(mean_quarterly_data_Q2,MARGIN = 2,period_vec,'/')
    mean_spend_data_Q2 <- data.frame(mean_spend_data_Q2)

    mean_spend_data_Q3 <- sweep(mean_quarterly_data_Q3,MARGIN = 2,period_vec,'/')
    mean_spend_data_Q3 <- data.frame(mean_spend_data_Q3)

    mean_spend_data_Q4 <- sweep(mean_quarterly_data_Q4,MARGIN = 2,period_vec,'/')
    mean_spend_data_Q4 <- data.frame(mean_spend_data_Q4)

    ##Lower and upper bounds of optimization
    ##By default use 10% lower values than the simulator values.
    optimization_lb_mat_Q1 <- var_optimization_lb * mean_spend_data_Q1
    optimization_ub_mat_Q1 <- var_optimization_ub * mean_spend_data_Q1

    ##Create dataframes of the optimization matrices to be joined for limits check later
    opt_lb_df_Q1 <- data.frame('dma' = dma_names_vec,optimization_lb_mat_Q1)
    opt_ub_df_Q1 <- data.frame('dma' = dma_names_vec,optimization_ub_mat_Q1)

    optimization_lb_mat_Q2 <- var_optimization_lb * mean_spend_data_Q2
    optimization_ub_mat_Q2 <- var_optimization_ub * mean_spend_data_Q2

    ##Create dataframes of the optimization matrices to be joined for limits check later
    opt_lb_df_Q2 <- data.frame('dma' = dma_names_vec,optimization_lb_mat_Q2)
    opt_ub_df_Q2 <- data.frame('dma' = dma_names_vec,optimization_ub_mat_Q2)

    optimization_lb_mat_Q3 <- var_optimization_lb * mean_spend_data_Q3
    optimization_ub_mat_Q3 <- var_optimization_ub * mean_spend_data_Q3

    ##Create dataframes of the optimization matrices to be joined for limits check later
    opt_lb_df_Q3 <- data.frame('dma' = dma_names_vec,optimization_lb_mat_Q3)
    opt_ub_df_Q3 <- data.frame('dma' = dma_names_vec,optimization_ub_mat_Q3)

    optimization_lb_mat_Q4 <- var_optimization_lb * mean_spend_data_Q4
    optimization_ub_mat_Q4 <- var_optimization_ub * mean_spend_data_Q4

    ##Create dataframes of the optimization matrices to be joined for limits check later
    opt_lb_df_Q4 <- data.frame('dma' = dma_names_vec,optimization_lb_mat_Q4)
    opt_ub_df_Q4 <- data.frame('dma' = dma_names_vec,optimization_ub_mat_Q4)


    opt_lb_df_all <- rbind(opt_lb_df_Q1, opt_lb_df_Q2, opt_lb_df_Q3, opt_lb_df_Q4)
    opt_ub_df_all <- rbind(opt_ub_df_Q1, opt_ub_df_Q2, opt_ub_df_Q3, opt_ub_df_Q4)

    opt_lb_df_all[opt_lb_df_all==0] <- NA
    opt_lb_df <- opt_lb_df_all %>% group_by(dma) %>% summarise(across(everything(), list(min), na.rm = TRUE)) %>% ungroup() %>% mutate_if(is.numeric, function(x) ifelse(is.infinite(x), 0, x)) %>% as.data.frame()

    # opt_lb_df <- opt_lb_df_all %>% group_by(dma) %>% summarise_each(funs(min(.[. != 0])))  %>% ungroup() %>% as.data.frame()
    opt_ub_df <- opt_ub_df_all %>% group_by(dma) %>% summarise(across(everything(), list(max))) %>% ungroup() %>% as.data.frame()
    colnames(opt_lb_df) <- colnames(opt_lb_df_all)
    colnames(opt_ub_df) <- colnames(opt_ub_df_all)

    optimization_lb_mat <- opt_lb_df %>% select(-dma) 
    optimization_ub_mat <- opt_ub_df %>% select(-dma)



    # return(list('mean_quarterly_data' = mean_quarterly_data,
