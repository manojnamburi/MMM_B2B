...
get_quarterly_mix <- function(input_quarterly_budget_opt,
  ...
  var_n_flight_period <- data.frame(matrix(nbr_period_q,nrow = length(dma_names_vec),ncol = length(media_names_vec)+1,dimnames = list(NULL,c(media_names_vec,target_var))))
...
#       exclude_flight_media <- names(var_n_flight_media_OPT)[which(var_n_flight_period_OPT == nbr_period_q)]
...
#       exclude_flight_media <- names(var_n_flight_media_SDL)[which(var_n_flight_period_SDL == nbr_period_q)]
...
#   if(((length(sim_results_opt[['budget']])-1)!=length(flight_media_vec_OPT))||(((length(sim_results_sdl[['budget']])-1)!=length(flight_media_vec_SDL)))){stop(cat('Please ensure you have entered the possible week combination values for the flight scenarios.'))}
   ...
   ##Construct the mean_spend_data from the individual result objects
   mean_spend_data <- data.frame(beta_media_mat*0) 
    
   mean_spend_data_opt <- sim_results_opt$Mean_Spend_Data
   mean_spend_data_sdl <- sim_results_sdl$Mean_Spend_Data
    
   if(is.null(mean_spend_data_opt)){stop(cat('Please provide the entire result object from the simulator which contains the mean spend data object for Optimum'))}
   if(is.null(mean_spend_data_sdl)){stop(cat('Please provide the entire result object from the simulator which contains the mean spend data object for Suddenlink'))}
    
   mean_spend_data[var_id_opt,] <- mean_spend_data_opt[var_id_opt,]
   mean_spend_data[-var_id_opt,] <- mean_spend_data_sdl[-var_id_opt,]
    
    
  ##Based on whether Smoothening results should be considered by footprint
  if(adjusted_res_OPT){
      sim_results_opt <- sim_results_opt[["adjusted_output"]]  
   }
  else if(smoothened_res_OPT){
      sim_results_opt <- sim_results_opt[["Post_smoothening_output"]]
   }else{
      sim_results_opt <- sim_results_opt[["Without_smoothening_output"]]
   }
    
  if(adjusted_res_SDL){
      sim_results_sdl <- sim_results_sdl[["adjusted_output"]]  
   }
   else if(smoothened_res_SDL){
      sim_results_sdl <- sim_results_sdl[["Post_smoothening_output"]]
   }else{
      sim_results_sdl <- sim_results_sdl[["Without_smoothening_output"]]
   }
  ...
  constr_budget_df <- data.frame(matrix(0,nrow = length(dma_names_vec),ncol = length(common_cols)-1))
  constr_target_df <- data.frame(matrix(0,nrow = length(dma_names_vec),ncol = length(common_cols)-1))
  ...
  constr_budget_df_on <- constr_budget_df
  constr_budget_df_off <- constr_budget_df

  constr_target_df_on <- constr_target_df
  constr_target_df_off <- constr_target_df

  constr_budget_mat_on <- 0*beta_media_mat
  constr_budget_mat_off <- 0*beta_media_mat
  ...
  if(!is.null(constr_media_vec_OPT)||!is.null(constr_media_vec_SDL)){
    ...
    ##Default case when there are no flight media present
    nbr_periods_on_opt <- nbr_period_q
    nbr_periods_on_sdl <- nbr_period_q
    
    ##If the flight media is present, then calculate the target seperately for on and off as budget levels differ and the spend is not a linear function of target
    if(!is.null(flight_media_vec_OPT)){
      nbr_periods_on_opt <- var_n_flight_period_OPT
    }
    
    if(!is.null(flight_media_vec_SDL)){
      nbr_periods_on_sdl <- var_n_flight_period_SDL
    }
    ##Number of periods when the flight media are off
    nbr_periods_off_opt <- nbr_period_q - nbr_periods_on_opt
    nbr_periods_off_sdl <- nbr_period_q - nbr_periods_on_sdl

    cat(sprintf('Periods off OPT:%s\n',nbr_periods_off_opt))
    cat(sprintf('Periods off SDL:%s\n',nbr_periods_off_sdl))

    constr_budget_period_OPT_on <- constr_media_budget_OPT/nbr_periods_on_opt
    constr_budget_period_SDL_on <- constr_media_budget_SDL/nbr_periods_on_sdl


    #constr_budget_period_OPT_off <- ifelse(nbr_periods_off_opt==0,0,(constr_media_budget_OPT/nbr_periods_off_opt))
    #constr_budget_period_SDL_off <- ifelse(nbr_periods_off_sdl==0,0,(constr_media_budget_SDL/nbr_periods_off_sdl))


    if(nbr_periods_off_opt==0){
      constr_budget_period_OPT_off <- 0
    }else{
      constr_budget_period_OPT_off <- (constr_media_budget_OPT/nbr_periods_off_opt)
    }

    if(nbr_periods_off_sdl==0){
      constr_budget_period_SDL_off <- 0
    }else{
      constr_budget_period_SDL_off <- (constr_media_budget_SDL/nbr_periods_off_sdl)
    }
    ...
    constr_budget_mat_on[var_id_opt,constr_media_vec_OPT] <- constr_budget_period_OPT_on
    constr_budget_mat_on[-var_id_opt,constr_media_vec_SDL] <- sweep(as.matrix(share_spend_sdl_mat[,constr_media_index_SDL]), MARGIN=2, constr_budget_period_SDL_on, `*`)

    constr_budget_mat_off[var_id_opt,constr_media_vec_OPT] <- constr_budget_period_OPT_off
    constr_budget_mat_off[-var_id_opt,constr_media_vec_SDL] <- sweep(as.matrix(share_spend_sdl_mat[,constr_media_index_SDL]), MARGIN=2, constr_budget_period_SDL_off, `*`)

    write.table(constr_budget_mat_off)

    ##Compute the target value for the constrained media
    constr_target_mat_on <- gross_add_weekly(solution = constr_budget_mat_on,
    ...
    constr_target_mat_off <- gross_add_weekly(solution = constr_budget_mat_off,
                                          min_media_mat = min_media_mat,
                                          max_media_mat = max_media_mat,
                                          ratio_dma = spend_activity_ratio_dma_df,
                                          y_max = target_max_vec,
    ...
    constr_target_by_dma_on <- rowSums(constr_target_mat_on)

    if(!(length(constr_target_by_dma_on)!=dim(constr_budget_mat_on)[2]))
    {
      stop(cat('Error in merging the constrained target to constrained budget for on condition. Check dimensions.'))
    }

    constr_budget_df_on[media_names_vec] <- constr_budget_mat_on
    constr_budget_df_on[target_var] <- constr_target_by_dma_on

    

    constr_target_df_on[media_names_vec] <- constr_target_mat_on
    constr_target_df_on[target_var] <- constr_target_by_dma_on

    ##When the flight media are off, compute the constrained budget and target
    constr_target_by_dma_off <- rowSums(constr_target_mat_off)

    if(!(length(constr_target_by_dma_off)!=dim(constr_budget_mat_off)[2]))
    {
      stop(cat('Error in merging the constrained target to constrained budget for off condition. Check dimensions.'))
    }

    constr_budget_df_off[media_names_vec] <- constr_budget_mat_off
    constr_budget_df_off[target_var] <- constr_target_by_dma_off

    #write.table(constr_budget_df_off)

    constr_target_df_off[media_names_vec] <- constr_target_mat_off
    constr_target_df_off[target_var] <- constr_target_by_dma_off
    ...
    names(constr_target_by_dma_on) <- dma_names_vec
    names(constr_target_by_dma_off) <- dma_names_vec
  ...
  all_on_budget_df <- all_on_budget_df + constr_budget_df_on
  ...
  all_on_target_df <- all_on_target_df + constr_target_df_on
  ...
  if(is.null(flight_media_vec_OPT)&&is.null(flight_media_vec_SDL)){
     quarterly_mix_df <- var_n_flight_period*all_on_budget_df
    quarterly_target_df <- var_n_flight_period*all_on_target_df
  }
  ...
  if(!is.null(flight_media_vec_OPT)||!is.null(flight_media_vec_SDL)){
    ...
    if(!is.null(flight_media_vec_OPT)){
        ##Rearrange the flight media vector similar to the media names irrespective of the order user specifies
        flight_media_vec_OPT <- media_names_vec[media_names_vec%in%flight_media_vec_OPT]
        var_n_flight_period[var_id_opt,media_names_vec] <- var_n_flight_period_OPT
    ...
    if(!is.null(flight_media_vec_SDL)){
        ##Rearrange the flight media vector similar to the media names irrespective of the order user specifies
        flight_media_vec_SDL <- media_names_vec[media_names_vec%in%flight_media_vec_SDL]
        var_n_flight_period[-var_id_opt,media_names_vec] <- var_n_flight_period_SDL
    ...
    all_off_budget_df <- all_off_budget_df + constr_budget_df_off
    ...
    all_off_target_df <- all_off_target_df + constr_target_df_off
  ...
  return(list('Formatted' = list('Footprint' = list('Spend' = fprint_spend_df,
              ...
              'Control_GA' = control_target_mat,
              'media_target_vec' = media_target_vec,
              'target_by_dma' = target_by_dma,
              'quarterly_target_df' = quarterly_target_df,
              'all_on_target_df' = all_on_target_df,
              'all_off_target_df' = all_off_target_df))
